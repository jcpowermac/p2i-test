...
RUN yum install -y  --setopt=tsflags=nodocs centos-release-openshift-origin openssh-server openssh-clients && \
    yum install -y --setopt=tsflags=nodocs origin-clients && \ 
    yum clean all -y && \
